...
# Update the scene here, this is called once each frame
  ...
  def pbDrawMoveList
    ...
    textpos=[
       [_INTL("¿Qué movimiento quieres que enseñe?"),16,8,0,Color.new(88,88,80),Color.new(168,184,184)]
    ]
    yPos=82
    ...
    textpos.push([_INTL("CATEGORÍA"),272,114,0,Color.new(248,248,248),Color.new(0,0,0)])
    textpos.push([_INTL("POTENCIA"),272,146,0,Color.new(248,248,248),Color.new(0,0,0)])
    textpos.push([basedamage<=1 ? basedamage==1 ? "???" : "---" : sprintf("%d",basedamage),
          468,146,2,Color.new(64,64,64),Color.new(176,176,176)])
    textpos.push([_INTL("PRECISIÓN"),272,178,0,Color.new(248,248,248),Color.new(0,0,0)])
...
class MoveRelearnerScreen
  ...
  def pbStartScreen(pokemon)
    ...
    loop do
      ...
      if move<=0
        if @scene.pbConfirm(
          _INTL("¿Prefieres que {1} no aprenda un movimiento nuevo?",pokemon.name))
          @scene.pbEndScene
          return false
        end
      else
        if @scene.pbConfirm(_INTL("¿Enseñar {1}?",PBMoves.getName(move)))
