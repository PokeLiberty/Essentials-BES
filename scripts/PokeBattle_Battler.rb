...
#        pbShowAnimation(@id,self,@battle.battlers[i],hitnum,alltargets,showanimation)
        showAbilityMessage(self)
        @battle.pbDisplay(_INTL("¡Se eliminaron todos los cambios de características!"))
        end
      end
    end
    # Descarga
    ...
    if self.hasWorkingAbility(:ELECTRICSURGE) && 
  ...
  def pbEffectsOnDealingDamage(move,user,target,damage)
    ...
    if damage>0 && move.isContactMove? && !user.hasWorkingAbility(:LONGREACH) && !user.hasWorkingItem(:PROTECTIVEPADS)
      if !target.damagestate.substitute
        ...
        if target.hasWorkingAbility(:GOOEY,true)                     # Baba
          if user.pbReduceStatWithCause(PBStats::SPEED,1,target,PBAbilities.getName(target.ability))
            PBDebug.log("[Habilidad disparada] Baba de #{target.pbThis}")
          end
        end
        if user.hasWorkingAbility(:POISONTOUCH,true) &&              # Toque Tóxico
    ...
    if damage>0
      ...
      if target.hasWorkingAbility(:ANGERPOINT)                       # Irascible
...
# TODO: "Before Protect" applies to Counter/Mirror Coat
    ...
    if target.hasWorkingAbility(:DAZZLING) && p>0 && !user.hasMoldBreaker      # Cuerpo Vívido
      @battle.pbDisplay(_INTL("¡Cuerpo Vívido impidió el ataque!",target.pbThis))
      PBDebug.log("[Movimiento falló] Regia Presencia bloqueó el ataque")
      return false
    end      
    if target.pbOwnSide.effects[PBEffects::WideGuard] &&             # Vastaguardia
  ...
  def pbProcessMoveAgainstTarget(thismove,user,target,numhits,turneffects,nocheck=false,alltargets=nil,showanimation=true,dancercheck=false)
    ...
    for i in 0...numhits
      ...
      if !nocheck &&
        ...
        elsif thismove.function==0x10B                     # Patada Salto Alta, Patada Salto
          if !user.hasWorkingAbility(:MAGICGUARD)
