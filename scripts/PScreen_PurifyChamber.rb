    ...
    for set in 0...NUMSETS
      ...
      if @sets[set].shadow
        if @sets[set].shadow.heartgauge>0
          ...
          if isPurifiable?(set)
            Kernel.pbMessage(_INTL("¡Tu {1} está en la Cámara de Purificación listo para ser purificado!",@sets[set].shadow.name))
          end
        end
      end
    end
...
class PurifyChamberPC
  ...
  def name
    return _INTL("Cámara de Purificación")
  end

  def access
    Kernel.pbMessage(_INTL("\\se[accesspc]Acceso a la Cámara de Purificación concedido."))
    pbPurifyChamber()
  end
end
...
#   end
  ...
  def pbPlace(pkmn,position)
    ...
    if pkmn.isEgg?
      @scene.pbDisplay(_INTL("No se puede colocar un huevo ahí."))
      return false
    end  
    if (position==0)
      if pkmn.isShadow?
        ...
        @scene.pbDisplay(_INTL("Solamente se puede poner un Pokémon Oscuro ahí."))
        return false
      end
    elsif (position>=1)
      if !pkmn.isShadow?
        ...
        @scene.pbDisplay(_INTL("No se puede colocar un Pokemon Oscuro ahí."))
        return false
      end
    end
    return true
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set)==0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(
         _INTL("Este {1} está listo para abrir su corazón. Sin embargo, debe haber al menos un Pokémon normal en la cámara para completar la ceremonia de purificación.",pkmn.name))
    end
  end
  ...
  def pbOpenSetDetail
    ...
    loop do
      ...
      if cmd[0]==0
        ...
        if curpkmn || heldpkmn
          commands=[_INTL("MOVER"),_INTL("DATOS"),
             _INTL("SACAR")]
          if curpkmn && heldpkmn
            commands[0]=_INTL("INTERCAMBIAR")
          elsif heldpkmn
            commands[0]=_INTL("COLOCAR")
          end
          cmdReplace=-1
          cmdRotate=-1
          if !heldpkmn && curpkmn && cmd[1]==0 &&
             @chamber[@chamber.currentSet].length>0
            commands[cmdRotate=commands.length]=_INTL("ROTAR")
          end
          if !heldpkmn && curpkmn
            commands[cmdReplace=commands.length]=_INTL("REMPLAZAR")
          end
            commands.push(_INTL("SALIR"))
          choice=@scene.pbShowCommands(
             _INTL("¿Qué debería hacerse con {1}?",
          ...
          if choice==0
            ...
            if pbBoxesFull?
              @scene.pbDisplay("Todas las cajas están llenas.")
            elsif heldpkmn
            ...
            if pos
              ...
              if newpkmn
                if (newpkmn.isShadow?)!=(curpkmn.isShadow?)
                  @scene.pbDisplay(_INTL("Ese Pokémon no se puede colocar ahí."))
                else
        ...
        if heldpkmn
          @scene.pbDisplay("¡Estás sosteniendo un Pokémon!")
        else
          if !@scene.pbConfirm("¿Continuar editando la configuración?")
            break
          end
        end
      end
    ...
    if pbCheckPurify()
      @scene.pbDisplay(_INTL("¡Hay un Pokémon que está listo para abrir su corazón!\1"))
  ...
  def pbDoPurify
    ...
    for i in 0...purifiables.length
      ...
      if (i+1)!=purifiables.length
        @scene.pbDisplay(_INTL("¡Hay otro Pokémon que está listo para abrir su corazón!"))  
        if !@scene.pbConfirm("¿Quieres intercambiar las posiciones?")
          @scene.pbCloseSet()
          break
        end  
      end
  ...
  def pbStartPurify
    ...
    loop do
      ...
      if set<0
        if !@scene.pbConfirm("¿Continuar la vista de los hologramas?")
          break
        end
      else
        chamber.currentSet=set
        cmd=@scene.pbShowCommands(
           _INTL("¿Qué quieres hacer?"),[
           _INTL("EDITAR"),_INTL("CAMBIAR"),_INTL("SALIR")])
        if cmd==0 # edit
          if !pbOpenSetDetail()
            break
          end
...
class PurifyChamberSetView < SpriteWrapper
  ...
  def refresh
    ...
    if pkmn
        ...
        textpos.push([_INTL("{1}  Nv{2}  {3}",pkmn.name,pkmn.level,PBTypes.getName(pkmn.type1)),2,0,0,
             Color.new(248,248,248),Color.new(128,128,128)])
      else
        textpos.push([_INTL("{1}  Nv{2}  {3}/{4}",pkmn.name,pkmn.level,PBTypes.getName(pkmn.type1),
             PBTypes.getName(pkmn.type2)),2,0,0,
             Color.new(248,248,248),Color.new(128,128,128)])
      end
      textpos.push([_INTL("FLUJO"),2+@info.bitmap.width/2,24,0,
           Color.new(248,248,248),Color.new(128,128,128)])
          # dibuja medidor del corazón
        pbDrawGauge(@info.bitmap,Rect.new(@info.bitmap.width*3/4,8,@info.bitmap.width*1/4,8),
           Color.new(192,0,256),pkmn.heartgauge,
         PokeBattle_Pokemon::HEARTGAUGESIZE)
        # dibuja medidor de flujo
    ...
    if @chamber.setCount(@set)>0
      textpos.push([_INTL("RITMO"),2,24,0,
           Color.new(248,248,248),Color.new(128,128,128)])
      # dibuja medidor de ritmo
