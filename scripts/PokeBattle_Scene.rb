...
class FightMenuDisplay
  ...
  def refresh
    ...
    @buttons.refresh(self.index,@battler ? @battler.moves : nil,@megaButton,@ultraButton) if @buttons
  end
  ...
  def update
    ...
    if @buttons
      ...
      @buttons.update(self.index,moves,@megaButton,@ultraButton)
    end
  end
end
...
class FightMenuButtons < BitmapSprite
  ...
  def initialize(index=0,moves=nil,viewport=nil)
    ...
    @ultraburstbitmap=AnimatedBitmap.new(_INTL("Graphics/Pictures/cursor_ultra"))
  ...
  def update(index=0,moves=nil,megaButton=0,ultraButton=0)
    refresh(index,moves,megaButton,ultraButton)
  end

  def refresh(index,moves,megaButton,ultraButton)
    return if !moves
    self.bitmap.clear
...
class SafariDataBox < SpriteWrapper
  ...
  def refresh
    ...
    textpos.push([_INTL("Quedan: {1}",@battle.ballcount),30,38,false,base,shadow])
    pbDrawTextPositions(self.bitmap,textpos)
  end
