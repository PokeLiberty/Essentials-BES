...
################################################################################
  def pbTypeImmunityByAbility(type,attacker,opponent)
    ...
    if opponent.hasWorkingAbility(:SAPSIPPER) && isConst?(type,PBTypes,:GRASS)         # Herbívoro
    ...
    if (opponent.hasWorkingAbility(:STORMDRAIN) && isConst?(type,PBTypes,:WATER)) ||     # Colector
       (opponent.hasWorkingAbility(:LIGHTNINGROD) && isConst?(type,PBTypes,:ELECTRIC))   # Pararrayos
    ...
    if opponent.hasWorkingAbility(:MOTORDRIVE) && isConst?(type,PBTypes,:ELECTRIC)       # Electromotor
    ...
    if (opponent.hasWorkingAbility(:DRYSKIN) && isConst?(type,PBTypes,:WATER)) ||        # Piel Seca
       (opponent.hasWorkingAbility(:VOLTABSORB) && isConst?(type,PBTypes,:ELECTRIC)) ||  # Absorbe Elec.
       (opponent.hasWorkingAbility(:WATERABSORB) && isConst?(type,PBTypes,:WATER))       # Absorbe Agua
      PBDebug.log("[Habilidad disparada] #{PBAbilities.getName(opponent.ability)} de #{opponent.pbThis} (hizo ineficaz #{@name})")
    ...
    if opponent.hasWorkingAbility(:FLASHFIRE) && isConst?(type,PBTypes,:FIRE)            # Absorbe Fuego
    ...
    if opponent.hasWorkingAbility(:TELEPATHY) && pbIsDamaging? &&                        # Telepatía
    ...
    if opponent.hasWorkingAbility(:BULLETPROOF) && isBombMove?                           # Antibalas
...
################################################################################
  ...
  def pbCalcDamage(attacker,opponent,options=0)
    ...
    if @battle.field.effects[PBEffects::ElectricTerrain]>0 &&
       ...
       doesBypassIgnorableAbilities?) && isConst?(type,PBTypes,:ELECTRIC)
      damagemult=(damagemult*1.5).round
    end
