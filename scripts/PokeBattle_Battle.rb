...
#================================================================================
      ...
      if @opponent.is_a?(Array)
        raise _INTL("El oponente debe tener un sólo participante en batallas individuales") if @opponent.length!=1
        @opponent=@opponent[0]
      end
      if @player.is_a?(Array)
        raise _INTL("El jugador debe tener un sólo participante en batallas individuales") if @player.length!=1
        @player=@player[0]
      end
...
#===================================================================================
    if @doublebattle
      if @player.is_a?(Array)
        ...
        sendout2=pbFindNextUnfainted(@party1,pbSecondPartyBegin(0))
        raise _INTL("El jugador 2 no tiene ningún Pokémon saludable") if sendout2<0
        ...
        sendout2=pbFindNextUnfainted(@party1,sendout1+1)
        if sendout1<0 || sendout2<0
          raise _INTL("El jugador no tiene dos Pokémon saludables")
        end
        @battlers[0].pbInitialize(@party1[sendout1],sendout1,false)
        @battlers[2].pbInitialize(@party1[sendout2],sendout2,false)
