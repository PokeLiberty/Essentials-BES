...
class VoltorbFlip
  ...
  def pbNewGame
    ...
    pbDrawShadowText(@sprites["text"].bitmap,8,16,118,26,
       _INTL("Tus fichas"),Color.new(60,60,60),Color.new(150,190,170),1)
    pbDrawShadowText(@sprites["text"].bitmap,8,82,118,26,
       _INTL("Fichas ganadas"),Color.new(60,60,60),Color.new(150,190,170),1)
    # Draw current level
    pbDrawShadowText(@sprites["level"].bitmap,8,150,118,28,
       _INTL("Nivel {1}",@level.to_s),Color.new(60,60,60),Color.new(150,190,170),1)
    ...
    if $PokemonGlobal.coins>=MAXCOINS
      Kernel.pbMessage(_INTL("Tu Caja de fichas se ha llenado."))
      $PokemonGlobal.coins=MAXCOINS # As a precaution
      @quit=true
#    elsif !Kernel.pbConfirmMessage(_INTL("¿Jugar Voltorb Flip Nv. {1}?",@level)) && $PokemonGlobal.coins<99999
#      @quit=true
    else
  ...
  def getInput
    if Input.trigger?(Input::UP)
        ...
        for i in 0...@squares.length
          if @index[0]*64+128==@squares[i][0] && @index[1]*64==@squares[i][1] && @squares[i][3]==false
            ...
            if @squares[i][2]==0
              ...
              Kernel.pbMessage(_INTL("\\me[Voltorb Flip Game Over]¡Oh, no! ¡Terminaste con 0 fichas!\\wtnp[50]"))
              pbShowAndDispose
              @sprites["mark"].bitmap.clear
              if @level>1
                ...
                if @level>newLevel
                  @level=newLevel
                  @level=1 if @level<1
                  Kernel.pbMessage(_INTL("\\se[VoltorbFlipLevelDown]Bajó a juego Nv. {1}!",@level.to_s))
                end
              end
              # Update level text
      ...
      if count==0
        @sprites["curtain"].opacity=100
        Kernel.pbMessage(_INTL("\\me[Voltorb Flip Win]¡Mesa limpia!\\wtnp[40]"))
...
#        Kernel.pbMessage(_INTL("This means you've found all the Coins in this game, so the game is now over."))
        Kernel.pbMessage(_INTL("¡{1} ha recibido {2} fichas!",$Trainer.name,@points))
        # Update level text
        @sprites["level"].bitmap.clear
        pbDrawShadowText(@sprites["level"].bitmap,8,150,118,28,_INTL("Nivel {1}",@level.to_s),Color.new(60,60,60),Color.new(150,190,170),1)
        ...
        if @level<8
          @level+=1
          Kernel.pbMessage(_INTL("¡Avanzó a Juego Nv. {1}!",@level.to_s))
#          if @firstRound
#            Kernel.pbMessage(_INTL("¡Felicitaciones!"))
#            Kernel.pbMessage(_INTL("¡Puedes recibir aún más Monedas en el siguiente juego!"))
            @firstRound=false
#          end
        end
        pbDisposeSpriteHash(@sprites)
      ...
      @sprites["curtain"].opacity=100
      if @points==0
        if Kernel.pbConfirmMessage("¡No has encontrado ninguna Moneda! ¿Estás seguro que quieres salir?")
          @sprites["curtain"].opacity=0
         pbShowAndDispose
          @quit=true
        end
      elsif Kernel.pbConfirmMessage(_INTL("Si sales ahora, recibirás {1} Moneda(s). ¿Quieres salir?",@points))
        Kernel.pbMessage(_INTL("¡{1} ha recibido {2} Moneda(s)!",$Trainer.name,@points))
        $PokemonGlobal.coins+=@points
        @points=0
        pbUpdateCoins
        @sprites["curtain"].opacity=0
