...
class PokemonStorage
  ...
  def party=(value)
    raise ArgumentError.new("Sin soporte")
  end
  ...
  def initialize(maxBoxes=STORAGEBOXES,maxPokemon=30)
    ...
    for i in 0...maxBoxes
      ip1=i+1
      @boxes[i]=PokemonBox.new(_ISPRINTF("Caja {1:d}",ip1),maxPokemon)
      backid=i%24
...
=begin
  ...
  helptext=[
     _INTL("Mueve un Pokémon de una Caja a tu equipo."),
     _INTL("Guarda un Pokémon de tu equipo en una Caja."),
     _INTL("Organiza los Pokémon de las Cajas y tu equipo."),
     _INTL("Mueve los objetos que llevan los Pokémon de la Cajas y tu equipo."),
     _INTL("Regresar al menú anterior."),
  ]
...
=end
  ...
  def pbBoxCommands
    ...
    when 1
      ...
      wpaper=pbShowCommands(_INTL("Elige un fondo."),commands)
      if wpaper>=0
  ...
  def pbChoosePokemon(party=nil)
    ...
    loop do
      ...
      if selected==nil
        if pbConfirm(_INTL("¿Hacer más cambios?"))
          next
        else
          break
        end
  ...
  def pbUpdateOverlay(selection,party=nil)
    ...
    if !pokemon.isEgg?
      ...
      if pokemon.ability>0
        ...
        textstrings.push([_INTL("Sin habilidad"),36,234,false,nonbase,nonshadow])
      end
      if pokemon.item>0
        textstrings.push([PBItems.getName(pokemon.item),85,342,2,base,shadow])
      else
        textstrings.push([_INTL("Sin objeto"),85,342,2,nonbase,nonshadow])
      end
      if pokemon.isShiny?
...
class RegionalStorage
  ...
  def getCurrentStorage
    if !$game_map
      raise _INTL("El jugador no está en el mapa, por lo que no se puede determinar la región.")
    end
    ...
    if @rgnmap<0
      raise _INTL("El mapa actual no pertenece a ninguna región. Por favor, configura los metadatos MapPosition para este mapa.")
    end
    if !@storages[@rgnmap]
