...
def Kernel.pbConfirmMessage(message,&block)
  return (Kernel.pbMessage(message,[_INTL("Sí"),_INTL("No")],2,&block)==0)
end

def Kernel.pbConfirmMessageSerious(message,&block)
  return (Kernel.pbMessage(message,[_INTL("No"),_INTL("Sí")],1,&block)==1)
end
...
#BES-T Nametags de Polectron
def pbDisplayNametagWindow(msgwindow, name)
  namewindow=Window_AdvancedTextPokemon.new(_INTL("{1}",name))

  skinfile=MessageConfig.pbGetSpeechFrame()
  namewindow.setSkin(skinfile)
  
  namewindow.resizeToFit(namewindow.text,Graphics.width)
  namewindow.width=160 if namewindow.width<=160
  if msgwindow.y==0
...
def Kernel.pbMessageDisplay(msgwindow,message,letterbyletter=true,commandProc=nil)
  ...
  textchunks=[]
  controls=[]
  while text[/(?:\\([WwFf]|[Ff][Ff]|[Tt][Ss]|[Cc][Ll]|[Mm][Ee]|[Ss][Ee]|[Tt][Gg]|[Ww][Tt]|[Ww][Tt][Nn][Pp]|[Cc][Hh])\[([^\]]*)\]|\\([Gg]|[Cc][Nn]|[Ww][Dd]|[Ww][Mm]|[Oo][Pp]|[Cc][Ll]|[Ww][Uu]|[\.]|[\|]|[\!]|[\x5E])())/i]
  ...
  begin
    ...
    for i in 0...controls.length
      if controls[i] && controls[i][2]<=msgwindow.position && msgwindow.waitcount==0
        ...
        when "tg" #Display nametag by polectron
          namewindow.dispose if namewindow
          namewindow=pbDisplayNametagWindow(msgwindow, param)
    ...
    ##Text Skip##############################################
    if Input.press?(Input::L) && ($DEBUG || SKIPTEXT_DEBUG)
      msgwindow.textspeed=-999
      msgwindow.update
      if msgwindow.busy?
