...
class HallOfFameScene
  ...
  def writeTrainerData
    ...
    lefttext= _INTL("Nombre<r>{1}<br>",$Trainer.name)
    lefttext+=_INTL("ID N°<r>{1}<br>",pubid)
  ...
  def writePokemonData(pokemon,hallNumber=-1)
    ...
    pokename=_INTL("Huevo")+"/"+_INTL("Huevo") if pokemon.isEgg?
    idno=(pokemon.ot=="" || pokemon.isEgg?) ? "?????" : 
       sprintf("%05d",pokemon.publicID)
    dexnumber=pokemon.isEgg? ? _INTL("N° ???") : _ISPRINTF("No. {1:03d}",pokemon.species)
    textPositions=[
       ...
       [_INTL("Nv. {1}",pokemon.isEgg? ? "?" : pokemon.level),
           64,Graphics.height-48,0,BASECOLOR,SHADOWCOLOR],
       [_INTL("ID N°{1}",pokemon.isEgg? ? "?????" : idno),
           Graphics.width-192,Graphics.height-48,2,BASECOLOR,SHADOWCOLOR]
    ]
    if (hallNumber>-1)
      textPositions.push([_INTL("Hall de la Fama N°"),Graphics.width/2-104,0,0,BASECOLOR,SHADOWCOLOR])
...
class HallOfFamePC
  ...
  def name
    return _INTL("Hall de la Fama")
  end

  def access
    Kernel.pbMessage(_INTL("\\se[accesspc]Acceso al Hall de la Fama concedido."))
    pbHallOfFamePC
  end
end
