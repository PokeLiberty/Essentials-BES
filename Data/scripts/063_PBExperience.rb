module PBExperience
  ...
  MAXLEVEL=MAXIMUMLEVEL # Vea la configuración para cambiar el nivel máximo
...
# Obtiene los puntos de exp. máximos para el ratio de crecimiento dado.
# growth -- Ratio de crecimiento
...
# Obtiene la cantidad de puntos de exp. necesaria para alcanzar el nivel dado
# según el ratio de crecimiento indicado.
# growth -- Ratio de crecimiento
  def PBExperience.pbGetStartExperience(level,growth)
    ...
    if level<0
      return ArgumentError.new("El nivel es inválido.")
    end
    level=MAXLEVEL if level>MAXLEVEL
    return pbGetExpInternal(level,growth)
  end

# Agrega puntos de experiencia comprobando que la suma no exceda el máximo
# correspondiente al ratio de crecimiento dado.
# currexp -- Puntos de Exp actuales
# expgain -- Puntos de Exp a agregar
# growth -- Ratio de crecimiento
...
# Calcula el nivel a partir de los puntos de exp. y ratio de crecimiento dados.
# growth -- Ratio de crecimiento
