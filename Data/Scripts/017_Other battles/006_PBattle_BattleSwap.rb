class BattleSwapScene
  ...
  def pbSwapChosen(pkmnindex)
    ...
    commands.push(_INTL("POKÉMON INTERCAMBIO"))
    commands.push(_INTL("Salir"))
    @sprites["help"].text=_INTL("Selecciona el Pokémon para aceptar.")
    @sprites["list"].commands=commands
  ...
  def pbInitSwapScreen
    ...
    commands.push(_INTL("SALIR"))
    @sprites["help"].text=_INTL("Seleccionar el Pokémon para intercambiar.")
    @sprites["list"].commands=commands
    @sprites["list"].index=0
    @mode=1  
  end
  ...
  def pbStartSwapScene(currentPokemon,newPokemon)
    ...
    @sprites["title"]=Window_UnformattedTextPokemon.newWithSize(
       _INTL("INTERCAMB. POKÉMON"),0,0,Graphics.width,64,@viewport)
  ...
  def pbStartRentScene(rentals)
    ...
    @viewport.z=99999
    @sprites["title"]=Window_UnformattedTextPokemon.newWithSize(
       _INTL("POKéMON PRESTADO"),0,0,Graphics.width,64,@viewport)
    @sprites["help"]=Window_UnformattedTextPokemon.newWithSize("",
       0,Graphics.height-64,Graphics.width,64,@viewport)
    @sprites["list"]=Window_AdvancedCommandPokemonEx.newWithSize(
       [],0,64,Graphics.width,Graphics.height-128,@viewport)
...
class BattleSwapScreen
    ...
    loop do
      ...
      if command==0
        ...
        if chosen.any?{|item| item==index }
          ...
          @scene.pbUpdateChoices(chosen.clone)
          if chosen.length==3
            if @scene.pbConfirm(_INTL("¿Estos tres Pokémon están bien?"))
              retval=[]
  ...
  def pbStartSwap(currentPokemon,newPokemon)
    ...
    loop do
      ...
      if pkmn>=0
        commands=[_INTL("DATOS"),_INTL("CAMBIO"),_INTL("SEL. OTRO")]
        command=@scene.pbShowCommands(commands)
        if command==0
