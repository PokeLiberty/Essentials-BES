...
def pbCompileTrainerEvents(mustcompile)
  mapdata=MapData.new
  if File.exists?("Data/MapChecker.dat")
    maps = load_data("Data/MapChecker.dat")
  else
    maps = []
  end
  t = Time.now.to_i
  ...
  for id in mapdata.mapinfos.keys.sort
    ...
    oldid = maps[id]
    maps[id] = File.mtime(mapdata.mapFilename(id)).to_i #Modification time
    next if !map || !mapdata.mapinfos[id]
    if oldid != nil
      next if maps[id] <= oldid
    end
    #p "#{id}"
    ...
    for key in map.events.keys
      ...
      if TRAINER_COMPILATION #Desactivado
...
def pbCompileCommonEvents(mustcompile)
  mapdata=MapData.new
  Graphics.update
  changed=false
  ...
  pbEachPage(event) do |page|
    ...
    while i<list.length
      ...
      if list[i].code==655
        ...
        if DOOR_COMPILATION #BUG PUERTAS
        if params[0]==0
      ...
      elsif list[i].code==101 && TEXT_COMPILATION
...
def pbImportNewMaps
  ...
  Dir.chdir("Data"){
     mapdata=sprintf("Map*.rxdata","rxdata")
...
def pbCompileAllData(mustcompile)
  ...
  if mustcompile
    ...
    pbCompileTypes        # No dependencies
    yield(_INTL("Compilando datos del mapa de la región"))
    pbCompileTownMap      # No dependencies
    ...
    pbCompileItems        # Depends on PBMoves
    yield(_INTL("Compilando datos de las plantas de bayas"))
    pbCompileBerryPlants  # Depends on PBItems
    yield(_INTL("Compilando datos de los Pokemon"))
    pbCompilePokemonData  # Depends on PBMoves, PBItems, PBTypes, PBAbilities
    yield(_INTL("Compilando datos de las máquinas"))
    pbCompileMachines     # Depends on PBSpecies, PBMoves
    yield(_INTL("Compilando datos de los entrenadores"))
    pbCompileTrainers     # Depends on PBSpecies, PBItems, PBMoves
    yield(_INTL("Compilando datos del celular"))
    pbCompilePhoneData    # Depends on PBTrainers
    yield(_INTL("Compilando metadatos"))
    pbCompileMetadata     # Depends on PBTrainers
    yield(_INTL("Compilando datos de batallas con entrenadores"))
    pbCompileTrainerLists # Depends on PBTrainers
    yield(_INTL("Compilando datos de los encuentros"))
    pbCompileEncounters   # Depends on PBSpecies
    yield(_INTL("Compilando datos de los movimientos oscuros"))
    pbCompileShadowMoves  # Depends on PBSpecies, PBMoves
    yield(_INTL("Compilando animaciones"))
    pbCompileAnimations
    ...
    for i in 0...datafiles.length
      begin
        File.open("Data/#{datafiles[i]}"){|file|
           time = File.mtime("Data/#{datafiles[i]}").to_i
           latestdatatime=[latestdatatime,time].max
        }
    ...
    for i in 0...textfiles.length
      begin
        File.open("PBS/#{textfiles[i]}"){|file|
           time = File.mtime("PBS/#{textfiles[i]}").to_i
           latesttexttime=[latesttexttime,time].max
        }
      rescue SystemCallError
      end
    end
