  ...
  def pbPinchChange(idxPokemon)
    ...
    if !thispkmn.effects[PBEffects::Pinch] && thispkmn.status!=PBStatuses::SLEEP && 
      ...
      if nature==PBNatures::QUIET|| 
        ...
        pbDisplay(_INTL("¡{1} está ansioso por más!",thispkmn.pbThis))
      end
      if nature==PBNatures::CAREFUL||
        ...
        pbDisplay(_INTL("{1} comenzó gruñendo profundamente!",thispkmn.pbThis))
      end
      if nature==PBNatures::GENTLE||
        ...
        pbDisplay(_INTL("¡Un destello apareció en los ojos de {1}!",thispkmn.pbThis(true)))
      end
      if nature==PBNatures::JOLLY||
        ...
        pbDisplay(_INTL("¡{1} está tomando una posición!",thispkmn.pbThis))
      end
    end
  end
