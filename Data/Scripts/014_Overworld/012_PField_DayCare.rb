...
def pbDayCareWithdraw(index)
  if !$PokemonGlobal.daycare[index][0]
    raise _INTL("Aquí no hay ningún Pokémon...") if $DEBUG
  ...
  #  raise _INTL("No se puede guardar el Pokémon...")
    ...
    pbNicknameAndStore(pokemon,false)
...
def pbDayCareGenerateEgg
  ...
  if isConst?(babyspecies,PBSpecies,:RATTATA) ||
     ...
     isConst?(babyspecies,PBSpecies,:LILLIGANT) ||
     ...
     egg.form=mother.form if isConst?(mother.item,PBItems,:EVERSTONE)
  ...
  if !ditto0 || !ditto1
    ...
    if mother.species == father.species
      ...
      possible_balls.push(pokemon0.ballused) if pokemon0.isFemale? || ditto1
      possible_balls.push(pokemon1.ballused) if pokemon1.isFemale? || ditto0
    end
  ...
  egg.species = pbGetBabySpecies(egg.species)
  pbGenerateEgg(egg)

  #$Trainer.party[$Trainer.party.length]=egg
rescue
 p "Ups, el huevo no se genero correctamente"
end
   ...
   for i in 0...2
     ...
     if pkmn.exp<maxexp
       ...
       if pkmn.level!=oldlevel
         ...
         for i in movelist
           pkmn.pbLearnMove(i[1]) if i[0]==pkmn.level       # Aprendió un movimiento nuevo
         end
       end
     end
   end
