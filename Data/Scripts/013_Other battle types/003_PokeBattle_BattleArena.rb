class PokeBattle_BattleArena < PokeBattle_Battle
  ...
  def pbCanSwitchLax?(idxPokemon,pkmnidxTo,showMessages)
    if showMessages
      ...
      pbDisplayPaused(_INTL("¡{1} no puede ser remplazado!",thispkmn.pbThis))
    end
    return false
  end
  ...
  def pbEndOfRoundPhase
    ...
    if @count==3
      ...
      if points[0]==points[1]
        pbDisplay(_INTL("¡{1} empató con el {2} rival por decisión del árbitro!",
        ...
        pbDisplay(_INTL("¡{1} derrotó al {2} rival por decisión del árbitro!",
           @battlers[0].name,@battlers[1].name))
        @battlers[1].hp=0  
        @battlers[1].pbFaint(false)
      else
        pbDisplay(_INTL("¡{1} perdió ante el {2} rival por decisión del árbitro!",
           @battlers[0].name,@battlers[1].name))
        @battlers[0].hp=0  
        @battlers[0].pbFaint(false)
      end
  ...
  def pbBattleArenaJudgment(battler1,battler2,ratings1,ratings2)
    ...
    begin
      ...
      Kernel.pbMessageDisplay(msgwindow,
         _INTL("ÁRBITRO: ¡Suficiente! ¡Ahora haremos las evaluaciones para determinar al ganador!\\wtnp[20]")) {
