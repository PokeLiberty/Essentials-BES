module MessageConfig
  ...
  # en Graphics/Windowskins/  (especifique una cadena vacía para usar el tema por defecto)
  TextSkinName    = "speech hgss 1"
  ChoiceSkinName  = "choice 1"
  WindowOpacity   = 255
  TextSpeed       = nil # puede ser positivo para esperar frames o negativo
                        # para mostrar varios caracteres por cada frame
...
class AnimatedSprite < SpriteWrapper
  ...
  def initializeLong(animname,framecount,framewidth,frameheight,frameskip)
    ...
    raise _INTL("El ancho del frame es 0") if framewidth==0
    raise _INTL("El alto del frame es 0") if frameheight==0
    begin
    ...
    if @animbitmap.width%framewidth!=0
      raise _INTL("El ancho del bitmap ({1}) no es un múltiplo del ancho del frame ({2}) [Bitmap={3}]",
         @animbitmap.width,framewidth,animname)
    end
    if @animbitmap.height%frameheight!=0
      raise _INTL("El alto del bitmap ({1}) no es un múltiplo del alto del frame ({2}) [Bitmap={3}]",
  ...
  def initializeShort(animname,framecount,frameskip)
    ...
    if @animbitmap.width%framecount!=0
      raise _INTL("El ancho del bitmap ({1}) no es un múltiplo del contador de frame ({2}) [Bitmap={3}]",
         @animbitmap.width,framewidth,animname)
    end
    @framecount=framecount
