class MapBottomSprite < SpriteWrapper
  ...
  def refresh
    self.bitmap.clear
    if @nonests
      imagepos=[[sprintf("Graphics/#{POKEDEX_ROUTE}/pokedexNestUnknown"),108,172,0,0,-1,-1]]
    ...
    if @nonests
      textpos.push([_INTL("Zona desconocida"),Graphics.width/2,Graphics.height/2-16,2,
...
class PokemonRegionMapScene
  ...
  def pbStartScene(aseditor=false,mode=0)
    ...
    if !@map
      Kernel.pbMessage(_INTL("No se pudo encontrar el mapa."))
      return false
    end
    ...
    begin;drawRoamingPosition(mapindex);rescue;end
  ...
  def pbChangeMapLocation(x,y)
    ...
    currentname=Kernel.pbMessageFreeText(_INTL("Establecer el nombre para este lugar."),
